...
//postcon: If the denom is not equal to zero, 
//         the constructor initializes the Rational object with the provided num
//         and denom values and then calls the normalize() function to ensure the rational number is in its simplest form.
// 
//         If the denom is equal to zero, it prints an error message to cerr (standard error stream) and returns without initializing the object.
...
// 
//         If the denominator is negative, it negates both the numerator and denominator to ensure that the sign is in the numerator.
...
// 
//         If the products are not equal, the function returns false, indicating that the two rational numbers are not equal.
...
//         The function reads the numerator and denominator values from the input stream, separated by a slash(/ ) character.The format expected for input is numerator / denominator.
//         If the denominator is read as zero, it prints an error message to cerr(standard error stream) to indicate that the denominator cannot be zero.
//         After reading and validating the input, it calls the normalize() function to ensure that the Rational object is in its simplest form.
